...
class PoolInterface {
 public:
  ...
  static constexpr Offset kInsufficientSpace =
      std::numeric_limits<Offset>::max();//是64位int的最大值 要保留，因为这个值可能要返回给client FIXME 这里可能要改
  ...
  void prefetch_item(Offset offset) const;//prefetch那个item所在的cacheline以及接下来的cacheline

  char* get_item(Offset offset);
  const char* get_item(Offset offset) const;

  char* get_item(Offset offset, std::size_t* out_len);//只在set overwrite的时候调用 xhj
