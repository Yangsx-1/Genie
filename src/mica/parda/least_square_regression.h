...
uint64_t leastSquareRegression(float logX[], float logY[], uint64_t length,
                               struct avg* avgResult, struct skew* skewResult);
void averageLogData(float logX[], float logY[], uint64_t length,
                    struct avg* avgResult);
...
uint64_t leastSquareRegression(float logX[], float logY[], uint64_t length,
  ...
  for (uint64_t i = 0; i < length; i++) {
    skewNumerator +=
        (logX[i] - avgResult->avgLogX) * (logY[i] - avgResult->avgLogY);
...
void logTransform(uint32_t init[], float logX[], float logY[],
                  uint64_t length) {
...
double skewEstimation(uint32_t initAccessTime[], uint64_t length) {
  ...
  float logRank[30] = {0.000436, 0.693583, 1.099032, 1.386730, 1.609315,
                       1.792179, 1.945606, 2.079878, 2.196778, 2.302462,
                       2.398217, 2.485326, 2.565077, 2.638753, 2.707641,
                       2.773025, 2.832957, 2.889925, 2.944089, 2.995610,
                       3.044648, 3.091364, 3.135919, 3.178473, 3.219188,
                       3.258224, 3.295741, 3.331900, 3.366862, 3.400788};
  ...
  leastSquareRegression(logRank, logAccessTime, length, &avgResult,
                        &skewResult);
