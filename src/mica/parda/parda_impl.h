...
void STable::add_item(uint64_t keyhash, uint64_t curr_time, Bucket* curr_bucket,
                      size_t item_index) {
...
uint64_t get_pred_size(uint32_t* histogram, double target, uint64_t item_size) {
  ...
  while (histogram[last_bucket] == 0) last_bucket--;

  for (size_t i = 0; i <= last_bucket; i++) sum += histogram[i];
...
double theta_calculation(STable* table) {
  uint32_t heap[30] = {0};
  for (uint64_t bucket_index = 0; bucket_index < table->num_buckets_;
       ++bucket_index) {
    for (uint64_t item_index = 0; item_index < item_number; ++item_index) {
      STable::hash_item tmp_item =
          table->buckets_[bucket_index].item_vec[item_index];
