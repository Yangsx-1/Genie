...
void MultilayerPerceptron(const double* input, const uint64_t input_length,
                          double* output);
...
double B_I_TO_1[8] = {  //updated
    -0.58291296, -0.70027457, 0.61276995,  0.21925417,
    0.85070828,  0.0227995,   -1.76590272, -0.77290206};
...
double W_1_TO_2[8][6] = {  //updated
    ...
    {-0.94156915, 0.36078367, -0.5436194, -0.24907279, -0.16353172,
     -1.3750272}};
...
void MultilayerPerceptron(const double* input, const uint64_t input_length,
  ...
  for (int i = 0; i < FIRST_HIDDEN_LAYER_DIM; ++i) {
    double temp = 0.;
    for (int j = 0; j < 5; ++j) temp += W_I_TO_1[j][i] * scaled->data[j];
