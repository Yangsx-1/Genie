...
int main() {
  ...
  ::mica::util::ZipfGen zg(num_items, zipf_theta, static_cast<uint64_t>(0));
  ...
  while (true) {
    ...
    *key_ptr = key_i;
    ...
    if (is_get) {
      ...
      Result out_result = table.get(key_hash, key, key_length, value,
                                    value_length, &out_value_length, true);
    } else {
      value_i = seq;
      Result out_result =
